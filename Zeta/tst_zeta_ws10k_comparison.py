#!/usr/bin/env python3
"""
COMPLETE TST RIEMANN HYPOTHESIS ANALYSIS
- Runs TST simulation for WS_10k network
...
class CompleteTSTAnalysis:
    ...
    def run_complete_analysis(self):
        ...
        print("🌌 TST RIEMANN HYPOTHESIS COMPLETE ANALYSIS")
        print("=" * 60)
        
        # 1. Run TST simulation for WS_10k
        print("\n🚀 Step 1: Running TST simulation for WS_10k")
        start_time = time.time()
        
        G = self.create_network('WS', 10000, 4, 0.1)
        operator = self.build_desynchronization_operator(G, {'A': 0.9, 'B': 1.1, 'C': 0.1})
        eigenvalues = self.compute_eigenvalues(operator, 500)
        ...
        print(f"   Broad band [0.45,0.55]: {tst_analysis['band_counts']['broad']}/500 ({tst_analysis['band_counts']['broad']/500*100:.1f}%)")
        ...
        # Use only the first 500 zeros for fair comparison
        zeta_imag = zeta_zeros[:500]  # Already imaginary parts
        ...
        self.results = {
            ...
            'tst_simulation': {
                ...
                'broad_band_percentage': float(tst_analysis['band_counts']['broad']/500*100),
        ...
        with open('tst_zeta_complete_analysis.json', 'w') as f:
        ...
        print(f"💾 Results saved to 'tst_zeta_complete_analysis.json'")
    ...
    def create_visualizations(self, zeta_imag, tst_mapped, zeta_bins, zeta_corr, tst_bins, tst_corr):
        ...
        plt.tight_layout()
        plt.savefig('tst_zeta_comparison.png', dpi=300, bbox_inches='tight')
        plt.close()
        
        print("✅ Visualizations saved to 'tst_zeta_comparison.png'")
    ...
    def print_summary(self):
        ...
        print(f"TST Simulation (WS_10k):")
        print(f"  • Broad band [0.45,0.55]: {tst['broad_band_count']}/500 ({tst['broad_band_percentage']:.1f}%)")
