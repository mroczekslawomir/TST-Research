#!/usr/bin/env python3
"""
COMPLETE TST RIEMANN HYPOTHESIS ANALYSIS FOR WS_15k
- Runs TST simulation for WS_15k network (15,000 nodes, 800 eigenvalues)
...
class CompleteTSTAnalysisWS15k:
    def __init__(self):
        self.results = {}
    ...
    def run_complete_analysis(self):
        """Runs the complete analysis pipeline for WS_15k"""
        print("🌌 TST RIEMANN HYPOTHESIS COMPLETE ANALYSIS FOR WS_15k")
        print("=" * 70)
        
        # 1. Run TST simulation for WS_15k
        print("\n🚀 Step 1: Running TST simulation for WS_15k")
        start_time = time.time()
        
        G = self.create_network('WS', 15000, 4, 0.1)
        operator = self.build_desynchronization_operator(G, {'A': 0.9, 'B': 1.1, 'C': 0.1})
        eigenvalues = self.compute_eigenvalues(operator, 800)
        ...
        print(f"   Broad band [0.45,0.55]: {tst_analysis['band_counts']['broad']}/800 ({tst_analysis['band_counts']['broad']/800*100:.1f}%)")
        ...
        # Use only the first 800 zeros for fair comparison
        zeta_imag = zeta_zeros[:800]  # Already imaginary parts
        ...
        self.results = {
            ...
            'tst_simulation': {
                ...
                'broad_band_percentage': float(tst_analysis['band_counts']['broad']/800*100),
        ...
        with open('tst_zeta_ws15k_complete_analysis.json', 'w') as f:
        ...
        print(f"💾 Results saved to 'tst_zeta_ws15k_complete_analysis.json'")
    ...
    def create_visualizations(self, zeta_imag, tst_mapped, zeta_bins, zeta_corr, tst_bins, tst_corr):
        ...
        ax1.legend()
        ax1.set_title('Distribution Comparison - WS_15k')
        ...
        if zeta_corr is not None and tst_corr is not None:
            ...
            ax2.legend()
            ax2.set_title('Pair Correlation Function - WS_15k')
        ...
        if len(zeta_gaps) > 0 and len(tst_gaps) > 0:
            ...
            ax3.legend()
            ax3.set_title('Gap Distribution - WS_15k')
        ...
        ax4.legend()
        ax4.set_title('Cumulative Distribution - WS_15k')
        ax4.grid(True, alpha=0.3)
        
        plt.tight_layout()
        plt.savefig('tst_zeta_ws15k_comparison.png', dpi=300, bbox_inches='tight')
        plt.close()
        
        print("✅ Visualizations saved to 'tst_zeta_ws15k_comparison.png'")
    ...
    def print_summary(self):
        ...
        print("\n🎯 COMPLETE ANALYSIS SUMMARY FOR WS_15k")
        print("=" * 60)
        
        tst = self.results['tst_simulation']
        comp = self.results['comparison']
        
        print(f"TST Simulation (WS_15k):")
        print(f"  • Broad band [0.45,0.55]: {tst['broad_band_count']}/800 ({tst['broad_band_percentage']:.1f}%)")
...
if __name__ == "__main__":
    analyzer = CompleteTSTAnalysisWS15k()
